...
public class MultiInstanceProcess {
	...
	public void deploymentProcessDefinition() {
		Deployment deployment = processEngine.getRepositoryService()// 与流程定义和部署对象相关的Service
				...
				.name("MultiInstanceProcess")// 添加部署的名称
				.addClasspathResource("diagrams/multiinstance.bpmn")// 从classpath的资源中加载，一次只能加载一个文件
				.addClasspathResource("diagrams/multiinstance.png")// 从classpath的资源中加载，一次只能加载一个文件
				.deploy(); // 完成部署
	...
	public void startProcessInstance() {
		// 流程定义的key
		String processDefinitionKey = "multiInstanceProcess";
		List<String> assigneeList = Lists.newArrayList("zhangsan","lisi","wangwu");
		...
		ProcessInstance pi = processEngine.getRuntimeService()// 与正在执行 的流程实例和执行对象相关的Service
				.startProcessInstanceByKey(processDefinitionKey,params); // 使用流程定义的key启动流程实例,key对应helloworld.bpmn文件中id的属性值，使用key值启动，默认是按照最新版本的流程定义启动
